...
Run this once in the **Supabase SQL editor** for project `weckowkvqpamnlcqwvfh`
(https://supabase.com/dashboard/project/weckowkvqpamnlcqwvfh/sql/new) to set up the
Purchase Management module, then reload the app.

- Runs inside a single `BEGIN/COMMIT` transaction — all-or-nothing.
- Idempotent (`IF NOT EXISTS` guards throughout) — safe to run more than once.
- Canonical copy also lives at `supabase/migrations/28_purchase_management.sql`.
